...
void UFloatingCharMovetComp::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations))
	{
		...
		FHitResult* DownwardSweepResult;
		ComputeFloorDist(UpdatedComponent->GetComponentLocation(), 200, 200, OutFloorResult, CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleRadius(), DownwardSweepResult);

		if (OutFloorResult.bBlockingHit > 0) { // then we are "grounded"
			CharacterOwner->ResetJumpState();  //lets us jump again

			Velocity.Z += (OutFloorResult.GetDistanceToFloor() - 200) * 1500;
		}
